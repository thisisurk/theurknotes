...
// Fallback when an item has no per-item destination yet — visitor still lands
// in the dense archive (lesson + meta + channels) and can find the item there.
const ARCHIVE_FALLBACK = "/what-i-do";
...
// Click target = item.href · external URL opens in a new tab · internal path
// gets client-side nav · falls back to /what-i-do archive when href is null.
export function PortfolioHomeCard({ item }: Props) {
  const accent = item.accent;
  const href = item.href ?? ARCHIVE_FALLBACK;
